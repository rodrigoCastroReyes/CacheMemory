...
/*Test*/
void generarGrafico(char *filePath, int sizeCache);
...
int main(int argc, char** argv) {
	...
	//generarGrafico(filename,sizeCache);
...
int i=1;
while(i<=1000){
       // testLRUAlgorithm(filePath,i);
testClockAlgorithm(filePath,i);
//testLRUKAlgorithm(filePath,i);
//testOptimalAlgorithm(filePath,i);
i=i+1;
    }
}
...
void testLRUAlgorithm(char *filePath, int sizeCache){
    ...
    char linea[1340], hres[20], mres[20], scache[20];
...
// ESCRIBIENDO EN ARCHIVO LOS HITS Y MISSES PARA GENERAR GRAFICO
/*
 sprintf(scache, "%d", sizeCache);
 sprintf(hres, "%d", hits);
 sprintf(mres, "%d", misses);
...
strcat(mres, "\n");
  	...
  	m = fopen ("misses.txt", "a+" );	
 	fputs(mres, m );
	 strcpy(mres, "");		
 	fclose ( m );	

// FIN DE ESCRITURA AL ARCHIVO
*/
...
void testOptimalAlgorithm(char *filePath,int sizeCache){
 ...
 FILE *h, *m;
    ...
    char hres[20], mres[20], scache[20];
